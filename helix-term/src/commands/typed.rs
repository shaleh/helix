    ...
    let columns = vec![
        ui::PickerColumn::new("status", |item: &PreviewItem, _: &()| {
            let label = match item.status {
                ScopeStatus::Defined => "\u{2713} Defined",
                ScopeStatus::Inherited => "\u{2191} Inherited",
                ScopeStatus::Missing => "\u{2717} Missing",
            };
...
fn theme_check(
    ...
    use helix_view::theme::{ScopeStatus, DOCUMENTED_SCOPES};
    ...
    for scope in DOCUMENTED_SCOPES {
        match theme.scope_status(scope.name) {
            ScopeStatus::Defined => defined += 1,
            ScopeStatus::Inherited => inherited += 1,
            ScopeStatus::Missing => {
                if scope.essential {
    ...
    if missing_essential.is_empty() {
        cx.editor.set_status(format!(
            "Theme '{}': {}/{} defined, {} inherited, all essential scopes covered",
            theme_name, defined, total, inherited,
        ));
    } else {
        cx.editor.set_error(format!(
            "Theme '{}': missing essential scopes: {}. Run :theme-preview for details.",
            theme_name,
